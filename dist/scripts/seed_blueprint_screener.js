...
const run = async () => {
    ...
    const screenerSection = await ScreenerSections.create('During the past TWO (2) WEEKS, how much (or how often) have you been bothered by the following problems?', 'standard', screener.id);
    screenerQuestions.forEach(async (question) => {
        await ScreenerQuestions.create(question.text, 'multi-choice', screenerSection.id, question.domain, screenerQuestionAnswers);
    });
};
run();
